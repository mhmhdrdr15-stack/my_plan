...
class AppDatabase {
  ...
  Future<void> _onCreate(
    ...
    await _createFoodEntriesTable(
      db,
    );

    await _createMealSchedulesTable(
      db,
    );

    await _createMealTemplatesTable(
      db,
    );

    await _createMealTemplateItemsTable(
      db,
    );

    await _createMealDayAssignmentsTable(
      db,
    );
  }
  ...
  Future<void> _createFoodEntriesTable(
    Database db,
  ) async {
    await db.execute('''
    ...
    // دعم قواعد البيانات القديمة التي أنشئت
    // قبل إضافة قيم الماكروز.
    await _ensureFoodEntryColumn(
      db,
      'protein_per_100g',
    );
  ...
  Future<void> _ensureFoodEntryColumn(
    Database db,
    String column,
  ) async {
    final columns =
  ...
  Future<void> _createMealSchedulesTable(
    Database db,
  ) async {
    await db.execute('''
  ...
  Future<void> _createMealTemplatesTable(
    Database db,
  ) async {
    await db.execute('''
  ...
  Future<void> _createMealTemplateItemsTable(
    Database db,
  ) async {
    await db.execute('''
  ...
  Future<void> _createMealDayAssignmentsTable(
    Database db,
  ) async {
    await db.execute('''
  ...
  Future<double>
      loadPlanningDailyCalories() async {
    final value =
        await _loadString(
      'planning_daily_calories',
    );
  ...
  Future<double>
      loadPlanningDailyProtein() async {
    final value =
        await _loadString(
      'planning_daily_protein',
    );
  ...
  Future<double>
      loadPlanningDailyCarbs() async {
    final value =
        await _loadString(
      'planning_daily_carbs',
    );
