...
class _LogFoodScreenState extends State<LogFoodScreen> {
  ...
  List<FoodEntry> _foodEntries = const [];
  ...
  void _handleAppStateChange() {
    if (!mounted) return;
    setState(() {
      _foodEntries = List<FoodEntry>.from(appState.foodEntries);
    });
  }

  Future<void> _loadFoodEntries() async {
    final entries = await appState.loadFoodEntries();
    if (!mounted) return;
    setState(() {
  ...
  Widget _foodList() {
    final entries = _foodEntries.isEmpty ? const <FoodEntry>[] : _foodEntries;
    return Container(
      decoration: BoxDecoration(
        color: Colors.white,
          ...
          : Column(
              children: [
                for (var i = 0; i < entries.length; i++)
                  _foodItem(entries[i], i == entries.length - 1),
              ],
            ),
    );
  }

  Widget _foodItem(FoodEntry food, bool last) {
    return Container(
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 4),
                Text(
                  '${food.mealType} • ${food.createdAt.toLocal().toString().substring(11, 16)}',
                  maxLines: 1,
                  overflow: TextOverflow.ellipsis,
